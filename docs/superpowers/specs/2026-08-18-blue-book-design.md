...
- **Look.** Booklet colours: baby blue (`#d9e7f6`) for the header block and the Hints button, a lighter cream (`#faf6ee`) for cards, navy type. No green/amber/red tier colours in the UI (the share-text emoji tiers remain for now). The end-of-day scorecard will later be modelled on an exam blue book cover.
- **Passages** display exactly as the source edition prints them; no dash conversion on render.
